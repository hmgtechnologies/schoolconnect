...
const Generator = {
  ...
  async build(config) {
    ...
    let logoExt = 'svg';
    if (cfg.logoData) {
      const match = cfg.logoData.match(/^data:image\/(png|jpeg|jpg);base64,(.*)$/);
      if (match) {
        logoExt = match[1] === 'jpeg' ? 'jpg' : match[1];
        zip.file('assets/img/logo.' + logoExt, match[2], {base64: true});
      } else {
    ...
    const pageIds = ['students','staff','classes','attendance','results','timetable','sow','cbt','assignments','library','conduct','health','promotion','fees','finance','leave','visitors','transport','announcements','events','messages','inbox','complaints','broadcast','gallery','eresources','birthdays','idcards','reports','directory','departments','parents','admissions','hr','hostel','alumni','inventory','certificates','analytics','lms','gamification','cafeteria','financial_aid','front_desk','career_counseling','document_builder','fleet_tracking','facility_booking','compliance',
      ...
      'timetable-generator','checkin','diary','surveys','menu','settings'];
...
const CORE = ['./','./index.html','./login.html','./dashboard.html','./assets/css/style.css','./assets/js/config.js','./assets/js/app.js','./assets/js/notifications.js','./assets/js/voting.js','./assets/js/pwa-install.js','./assets/js/super.js','./assets/js/cbt-engine.js','./assets/js/analytics.js','./assets/js/enterprise.js','./assets/img/logo.${cfg.logoExt}','./manifest.json'];
...
if(window.Super){ const p=document.getElementById('fl-preview'); if(p) p.innerHTML=Super.flyer.html(); }
  ...
  fullPreviewHtml(config) {
    ...
    const dedicated = {
      ...
      analytics: () => Generator.pageAnalytics(cfg), 'admin-data': () => Generator.pageAdminData(cfg),
...
const TTG={
  ...
  pageCheckin(cfg) {
    const body = `
      <div class="card" style="margin-bottom:16px"><p style="color:var(--gray-600);margin:0">
        Students check in by scanning their <strong>ID-card QR</strong> (or typing their admission number).
        Free — no biometric hardware required.</p></div>
      <div class="card">
        <div class="form-group"><label>Scan QR or type ID / admission no</label>
          <input class="form-input" id="ck-id" placeholder="ADM/1001 or paste QR text" autofocus></div>
        ...
        <button class="btn btn-primary" onclick="CK.go()">✅ Check in</button>
        <div id="ck-log" style="margin-top:16px"></div>
      </div>`;
    return T.shell(cfg, 'QR Check-in', body, { requireRole: 'any' })
      .replace('</body></html>', `<script>
const CK={ done:[],
  async go(){ const raw=document.getElementById('ck-id').value.trim(); if(!raw){toast('Scan or type an ID','warning');return;}
    const p=Enterprise.checkin.parseQR(raw);
    const r=await Enterprise.checkin.record(p.id, p.name||document.getElementById('ck-name').value, document.getElementById('ck-class').value, raw.startsWith('{')?'qr':'code');
    if(r&&r.error){toast(r.error.message||'Error','danger');return;}
    CK.done.unshift({id:p.id,name:p.name||document.getElementById('ck-name').value,at:new Date().toLocaleTimeString()});
    document.getElementById('ck-id').value=''; document.getElementById('ck-name').value=''; toast('Checked in ✓','success');
    document.getElementById('ck-log').innerHTML='<h3>Today\\'s check-ins</h3>'+CK.done.map(d=>'<div style="padding:6px 0;border-bottom:1px solid var(--gray-200)">✅ '+esc(d.id)+' '+esc(d.name||'')+' <span style="color:var(--gray-500)">'+d.at+'</span></div>').join(''); }
};
</script></body></html>`);
  },
