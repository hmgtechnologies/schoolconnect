...
<script>
  ...
  iconFor(id) {
    const map = {
      ...
      'report-cards':'🧾', 'admin-data':'🗄️', flyer:'📰', 'timetable-generator':'🗓️', checkin:'📲', diary:'📔', surveys:'🗒️', menu:'🍽️', settings:'⚙️'
    };
    return map[id] || '◦';
  },
  ...
  labelFor(id, fallbackName) {
    const map = {
      ...
      'admin-data':'Admin Data', flyer:'Flyer', checkin:'QR Check-in', diary:'Diary',
  ...
  modulePage(config, moduleId, opts = {}) {
    ...
    return T.shell(config, mod.name, `
      <p style="color:var(--gray-600);margin-bottom:24px">${T.esc(mod.desc || '')}</p>
      <div class="table-wrap"><table id="${T.esc(moduleId)}-table"><thead><tr><th>Loading…</th></tr></thead><tbody></tbody></table></div>
      <button class="btn btn-primary" style="margin-top:20px" onclick="App.openAddModal('${T.esc(moduleId)}')" data-admin-only>+ Add new</button>`,
